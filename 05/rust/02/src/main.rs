...
fn main() {
    let raw_rules_and_updates = fs::read_to_string("../../assets/test.txt").unwrap();
    ...
    let (processed_rules, incorrect_updates) = filter_updates(processed_rules, updates);

    let fixed_updates = fix_updates(processed_rules, incorrect_updates);
...
fn fix_updates(rules: HashMap<i32, Rule>, updates: Vec<Vec<i32>>) -> Vec<Vec<i32>> {
    updates
}
...
fn filter_updates(rules: HashMap<i32, Rule>, updates: Vec<Vec<i32>>) -> (HashMap<i32, Rule>, Vec<Vec<i32>>) {
