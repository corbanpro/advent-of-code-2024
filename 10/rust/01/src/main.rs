...
type TrailHead = ((i32, i32, i32), (i32, i32, i32));
...
fn main() {
    ...
    let mut trailheads: HashSet<TrailHead> = HashSet::new();
    ...
    for (y, row) in map.iter().enumerate() {
        ...
        for (x, height) in row.iter().enumerate() {
            let x = x as i32;
            let trailhead = (y, x, *height);
            if height == &0 {
                parse_neighbors(x, y, &map, trailhead, &mut trailheads)
            }
        }
    }
...
fn parse_neighbors(x: i32, y: i32, map: &[Vec<i32>], trailhead: (i32, i32, i32), trailheads: &mut HashSet<TrailHead>) {
    ...
    for x_move in [-1, 0_i32, 1] {
        for y_move in [-1, 0_i32, 1] {
            ...
            if neighbor_h == &9 {
                trailheads.insert((trailhead, (neighbor_y, neighbor_x, *neighbor_h)));
            } else {
                parse_neighbors(neighbor_x, neighbor_y, map, trailhead, trailheads)
            }
        }
    }
}
